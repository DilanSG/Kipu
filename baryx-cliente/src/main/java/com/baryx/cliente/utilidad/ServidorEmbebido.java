 ...
 * ejecutable nativo del servidor ({@code bin/BaryxServidor} en Linux,
 * {@code BaryxServidor.exe} en Windows) en la carpeta {@code servidor/}
 ...
 * {@code ~/.baryx/.env} que genera el script {@code setup-inicial.sh/.bat}.
...
public class ServidorEmbebido {
     ...
     */
    private static final String NOMBRE_EJECUTABLE_SERVIDOR = "BaryxServidor";
     ...
     * 3. Carga las variables de entorno desde ~/.baryx/.env
            ...
            try {
                ...
                if (rutaEjecutable == null) {
                    throw new RuntimeException(
                            "No se encontró el servidor Baryx. "
                ...
                // Esta URI viene como -Dbaryx.atlas.uri=... en el jpackage del cliente
    ...
    /**
     * Busca procesos Java del servidor Baryx en el sistema operativo.
    ...
    public static CompletableFuture<List<String[]>> buscarProcesosServidor() {
        return CompletableFuture.supplyAsync(() -> {
            ...
            logger.info("Procesos de servidor Baryx encontrados: {}", encontrados.size());
            return encontrados;
        });
    }

    /**
     * Determina si una línea de proceso corresponde al servidor Baryx.
     */
    private static boolean esProcesoServidor(String linea) {
        if (linea == null) return false;
        String lower = linea.toLowerCase();
        return (lower.contains("baryx") && lower.contains("servidor"))
                || lower.contains("baryx-servidor")
                || lower.contains("baryxservidor");
    }
     ...
     * con la estructura: servidor/bin/BaryxServidor (Linux) o
     * servidor/BaryxServidor.exe (Windows).
     * 
     * Orden de búsqueda:
    ...
    private static Path buscarEjecutableServidor() {
        ...
        // Linux/Mac: servidor/bin/BaryxServidor
        // Windows:   servidor/BaryxServidor.exe (en raíz del app-image)
        ...
        if (dirApp != null) {
            ...
            // Ej: /opt/baryx/cliente/baryxCliente/ y /opt/baryx/servidor/
            candidatas.add(dirApp.resolve("../../../" + NOMBRE_CARPETA_SERVIDOR));
        }
        ...
        if (esWindows) {
            // Windows: C:\Baryx\servidor\ o C:\Program Files\Baryx\servidor\
            String programFiles = System.getenv("ProgramFiles");
            if (programFiles != null) {
                candidatas.add(Paths.get(programFiles, "Baryx", NOMBRE_CARPETA_SERVIDOR));
            }
            candidatas.add(Paths.get("C:\\Baryx", NOMBRE_CARPETA_SERVIDOR));
            String localAppData = System.getenv("LOCALAPPDATA");
            if (localAppData != null) {
                candidatas.add(Paths.get(localAppData, "Baryx", NOMBRE_CARPETA_SERVIDOR));
            }
        } else {
            // Linux: /opt/baryx/servidor/
            candidatas.add(Paths.get("/opt/baryx", NOMBRE_CARPETA_SERVIDOR));
            candidatas.add(Paths.get("/opt/baryx/cliente", NOMBRE_CARPETA_SERVIDOR));
            // macOS: /Applications/Baryx/servidor/
            candidatas.add(Paths.get("/Applications/Baryx", NOMBRE_CARPETA_SERVIDOR));
            // Home del usuario
            String home = System.getProperty("user.home");
            if (home != null) {
                candidatas.add(Paths.get(home, "baryx", NOMBRE_CARPETA_SERVIDOR));
                candidatas.add(Paths.get(home, "Baryx", NOMBRE_CARPETA_SERVIDOR));
            }
        }
    ...
    /**
     * Carga las variables de entorno desde el archivo ~/.baryx/.env.
     * 
    ...
    private static Map<String, String> cargarVariablesEntorno() {
        ...
        Path[] candidatas = {
                Paths.get(System.getProperty("user.home"), ".baryx", ".env"),
