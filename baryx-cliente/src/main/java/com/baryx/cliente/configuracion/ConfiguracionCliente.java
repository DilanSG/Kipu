...
/* Configuración global del cliente Baryx. Esta clase centraliza la gestión de la configuración del cliente, incluyendo:
...
public class ConfiguracionCliente {
    ...
    private static final String NOMBRE_ARCHIVO_PROPIEDADES = "baryx-cliente.properties";
     ...
     * 2) system property -Dbaryx.atlas.uri (backward compat / override manual).
     *
    ...
    public static String getAtlasUriEmbebida() {
        ...
        try {
            String descifrada = com.baryx.common.utilidad.CifradoNube.leerDesdeClasspath();
        ...
        String prop = System.getProperty("baryx.atlas.uri", "");
        if (!prop.isEmpty()) {
            atlasUriCache = prop;
            return atlasUriCache;
        }

        // Intento 3: MONGODB_URI desde ~/.baryx/.env (desarrollo / config manual)
        try {
            java.nio.file.Path envPath = java.nio.file.Paths.get(
                    System.getProperty("user.home"), ".baryx", ".env");
    ...
    /*Carga la configuración desde el archivo baryx-cliente.properties. 
     * Busca el archivo en múltiples ubicaciones (en orden de prioridad):
     * 1. $HOME/.baryx/baryx-cliente.properties (ubicación principal para instalaciones)
    ...
    private static Path buscarArchivoPropiedades() {
        Path homeBaryx = Paths.get(System.getProperty("user.home"), ".baryx", NOMBRE_ARCHIVO_PROPIEDADES);
        if (Files.exists(homeBaryx)) {
            return homeBaryx;
        }
     ...
     * La configuración se guarda en $HOME/.baryx/baryx-cliente.properties para instalaciones, o en el directorio de trabajo actual para desarrollo/portable.
    ...
    private static void guardarPropiedades() {
        ...
        // Guardar siempre en $HOME/.baryx/ para instalaciones, y en el directorio de trabajo para desarrollo/portable
        Path dirBaryx = Paths.get(System.getProperty("user.home"), ".baryx");
        try {
            Files.createDirectories(dirBaryx);
        } catch (IOException e) {
            logger.error("No se pudo crear directorio de configuración {}: {}", dirBaryx, e.getMessage());
            return;
        }

        Path archivoProps = dirBaryx.resolve(NOMBRE_ARCHIVO_PROPIEDADES);
        try (OutputStream output = Files.newOutputStream(archivoProps)) {
            props.store(output, "Configuración del cliente Baryx - Generado automáticamente");
