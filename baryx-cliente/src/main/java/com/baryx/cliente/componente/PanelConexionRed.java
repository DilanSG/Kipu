...
public class PanelConexionRed {
    ...
    private VBox construirContenedorDatosNube() {
        ...
        campoBusinessId.textProperty().addListener((obs, old, val) -> {
            if (val != null && !val.isBlank()) {
                campoDbNombre.setText("baryx_" + val.trim().toLowerCase().replaceAll("[^a-z0-9_]", "_"));
            }
        });
    ...
    private void guardarConfigAtlas() {
        ...
        if (dbNombre.isEmpty()) {
            dbNombre = "baryx_" + businessId.toLowerCase().replaceAll("[^a-z0-9_]", "_");
        }
    ...
    private String obtenerConfigAtlas(String clave) {
        Path envPath = Paths.get(System.getProperty("user.home"), ".baryx", ".env");
    ...
    private void guardarEnvNube(String businessId, String dbNombre, String uriManual) {
        Path envDir = Paths.get(System.getProperty("user.home"), ".baryx");
        ...
        try {
            ...
            sb.append("# Configuración Baryx - Generado automáticamente\n");
