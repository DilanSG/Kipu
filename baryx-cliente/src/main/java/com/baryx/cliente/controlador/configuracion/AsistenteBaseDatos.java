...
import com.baryx.cliente.configuracion.ConfiguracionBd;
import com.baryx.cliente.configuracion.ConfiguracionCliente;
import com.baryx.cliente.utilidad.DetectorDependencias;
...
public class AsistenteBaseDatos {
    ...
    private void mostrarPasoCrearBd() {
        ...
        grid.add(crearCampoConDefault(campoDbNombre, "baryx_db"), 1, 0);
        grid.add(crearLabel(IdiomaUtil.obtener("asistente.pg.usuario_app")), 0, 1);
        grid.add(crearCampoConDefault(campoDbUsuario, "baryx_admin"), 1, 1);
    ...
    private void finalizarSetup() {
        ...
        CompletableFuture.runAsync(() -> {
            try {
                ...
                env.append("# Configuración Baryx - Generado por asistente GUI\n");
                ...
                if (!businessId.isEmpty()) {
                    ...
                    if (!atlasUri.isEmpty()) {
                        // Derivar nombre de DB en Atlas: baryx_{businessId}
                        env.append("MONGODB_DB_NAME='baryx_").append(
                                businessId.toLowerCase().replaceAll("[^a-z0-9_]", "_")).append("'\n");
                        // La URI se pasa como variable de entorno al subprocess del servidor
                    }
                }

                Path envFile = dirBaryx.resolve(".env");
    ...
    private void mostrarResumenFinal() {
        ...
        resumen.add(crearResumenValor("~/.baryx/.env"), 1, fila);
    ...
    private void exportarConfiguracion() {
        ...
        chooser.setInitialFileName("baryx-config-" +
        ...
        try {
            ...
            sb.append("# ================================================\n");
            sb.append("# Baryx - Configuración de Base de Datos\n");
            ...
            sb.append(".env       = ~/.baryx/.env\n");
            sb.append("Propiedades= ~/.baryx/baryx-cliente.properties\n");
            sb.append("\n");
            sb.append("# NOTA: Las contraseñas NO se incluyen en este archivo por seguridad.\n");
            sb.append("# Si necesita las credenciales, revise ~/.baryx/.env\n");
    ...
    private void mostrarCrearBdSimplificado() {
        ...
        grid.add(crearCampoConDefault(campoNombreBd, "baryx_db"), 1, fila++);
    ...
    private void guardarArchivoCredenciales(String nombreBd, String password, String negocio) {
        ...
        chooser.setInitialFileName("baryx-credenciales-" +
                LocalDateTime.now().format(DateTimeFormatter.ofPattern("yyyyMMdd-HHmmss")) + ".txt");
        chooser.getExtensionFilters().add(
                new FileChooser.ExtensionFilter("Text Files", "*.txt"));
        ...
        try {
            StringBuilder sb = new StringBuilder();
            sb.append("# ================================================\n");
            sb.append("# Baryx - Credenciales de Base de Datos\n");
            sb.append("# Generado: ").append(LocalDateTime.now().format(
                    DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss"))).append("\n");
    ...
    private void actualizarEnv(ConfiguracionBd bd) {
        try {
            Path envFile = Paths.get(System.getProperty("user.home"), ".baryx", ".env");
            ...
            for (String linea : lineas) {
                ...
                if (lt.startsWith("DB_HOST=")) {
                    ...
                    String bId = bd.getBusinessId();
                    if (bId != null && !bId.isEmpty()) {
                        nuevo.append("MONGODB_DB_NAME='baryx_").append(
    ...
    private void actualizarMongoDbNameEnEnv(String negocio) {
        try {
            ...
            String dbName = "baryx_" + negocio.toLowerCase().replaceAll("[^a-z0-9_]", "_");
            List<String> lineas = Files.readAllLines(envFile);
            StringBuilder nuevo = new StringBuilder();
            boolean encontrado = false;
            for (String linea : lineas) {
