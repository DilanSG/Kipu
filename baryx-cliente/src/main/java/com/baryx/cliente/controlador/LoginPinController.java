...
import com.baryx.cliente.servicio.AutenticacionServicio;
...
import com.baryx.cliente.utilidad.TecladoNumerico;
import com.baryx.cliente.componente.FondoAnimado;
...
/**
 * Controlador unificado para la vista de login del sistema Baryx.
 * 
...
public class LoginPinController {
    ...
    /**
     * Configura el efecto de hover en el logo BARYX.
        ...
        if (esHost && desconectado) {
            ...
            indicador.addEventHandler(javafx.scene.input.MouseEvent.MOUSE_CLICKED, e -> {
                ...
                if (raiz instanceof javafx.scene.layout.StackPane raizStack) {
                    com.baryx.cliente.componente.TopologiaRed
    ...
    private void sincronizarIdiomaDesdeServidor(String token) {
        CompletableFuture.runAsync(() -> {
            try {
                var servicio = new com.baryx.cliente.servicio.ConfiguracionSistemaServicio();
