; =========================================================
; Baryx Cliente - Script de Inno Setup
; =========================================================
...
;   /DAppPublisher=Baryx         Nombre del publicador
...
;   /DIconFile=C:\...\baryx.ico  (Opcional) Ruta al icono .ico
;   /DLicenseFile=C:\...\LICENSE (Opcional) Ruta al archivo de licencia
;
; Uso manual (sin package.ps1):
;   iscc /DAppVersion=1.0.0 /DSourceDir="C:\ruta\appimage\Baryx" /DServerDir="C:\ruta\dist\servidor" /DOutputDir="C:\salida" baryx-cliente.iss
; =========================================================
...
AppSupportURL=https://github.com/baryx
AppUpdatesURL=https://github.com/baryx

; Directorio de instalacion por defecto.
; El cliente se instala en Baryx\Cliente y el servidor en Baryx\servidor
; para que el Host Mode encuentre el servidor automaticamente.
DefaultDirName={autopf}\Baryx\Cliente
DefaultGroupName=Baryx
...
OutputDir={#OutputDir}
OutputBaseFilename=Baryx-{#AppVersion}
...
spanish.ConsoleShortcut=Baryx Cliente (Consola de diagnostico)
english.ConsoleShortcut=Baryx Client (Diagnostic Console)
spanish.ServerComponent=Servidor Baryx (necesario para Host Mode)
english.ServerComponent=Baryx Server (required for Host Mode)
...
; =========================================================
[Components]
Name: "principal"; Description: "Aplicacion Baryx Cliente"; Types: completa personalizada; Flags: fixed
...
; Servidor portable (se instala junto al cliente: Baryx\servidor\)
...
[Icons]
; Menu Inicio
Name: "{group}\Baryx Cliente"; Filename: "{app}\{#AppExeName}"; Tasks: startmenuicon
...
Name: "{group}\Desinstalar Baryx"; Filename: "{uninstallexe}"; Tasks: startmenuicon

; Escritorio
Name: "{autodesktop}\Baryx Cliente"; Filename: "{app}\{#AppExeName}"; Tasks: desktopicon

; =========================================================
; [Run] - Acciones post-instalacion
; =========================================================
[Run]
Filename: "{app}\{#AppExeName}"; Description: "{cm:LaunchApp,Baryx Cliente}"; Flags: nowait postinstall skipifsilent
...
begin
  ...
  begin
    if MsgBox('Se detecto una version anterior de Baryx Cliente instalada.' + #13#10 +
...
// Crear directorio .baryx en el perfil del usuario si no existe
...
begin
  ...
  begin
    BaryxDir := ExpandConstant('{userappdata}\..\..\.baryx');
    if not DirExists(BaryxDir) then
    begin
      CreateDir(BaryxDir);
    end;
  end;
